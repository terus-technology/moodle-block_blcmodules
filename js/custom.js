...
(function (jQuery) {
     ...
     function fillSubject() {
			...
			jQuery.getJSON("../blocks/blc_modules/load_scormsubject.php?apikey="+encodeURIComponent(apikey), function( data ) {
                var items = [];
                //console.log(data);
                if(data.length==0){
     ...
     function checkVersion(id) {
			...
			jQuery.getJSON("../blocks/blc_modules/version_check.php?apikey="+encodeURIComponent(apikey)+"&id="+id, function( data ) {
                var items = [];
                jQuery.each( data, function( key, val ) {
     ...
     function fillscorm() {
			...
			if(scormsubject != 0){	
				jQuery.getJSON("../blocks/blc_modules/load_scormurls.php?apikey="+encodeURIComponent(apikey)+"&subject="+encodeURIComponent(scormsubject), function( data ) {
					var items = [];
					jQuery.each( data, function( key, val ) {
	...
	jQuery( document ).ready(function() {
		...
		if ( jQuery( "#addscorm" ).hasClass( "block_blc_modules" ) ) {
			...
			jQuery(".section.main").each(function( ) {			
				if(notifyeditingon==1)
					jQuery(this).append("<a style='float:right; margin-right:"+mright+"; cursor: pointer; color: #a864a8;' class='add-scrom' data-toggle='modal' data-target='#bsModal3'><img style='padding-bottom:3px;' src='../blocks/blc_modules/pix/blc.png'/>   Add BLC modules</a>");
			...
			jQuery(".course-content").on("click",".submitForm", function() {
				...
				jQuery.get("../blocks/blc_modules/load_scorm.php",
				{
					id: id,
					sectionNumber: x,
					scormurls : scormurls,
