...
class blcservice extends external_api {
    ...
    public static function fetch_scorm_data(string $apikey, string $url, string $token, string $domainname): ?array {
        ...
        // NOTE: Do NOT urlencode() $url here — moodle_url encodes parameters
        // automatically. Double-encoding causes %25xx in the query string,
        // which makes the remote API unable to find the SCORM record.
        $serverurl = new moodle_url($domainname . '/webservice/rest/server.php', [
            'wstoken' => $token,
            'wsfunction' => $functionname,
            'apikey' => $apikey,
